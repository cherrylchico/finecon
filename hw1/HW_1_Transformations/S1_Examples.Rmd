---
title: 'EF Session 1: Transformations into stationarity'
output:
...
## Aim: Given three time series (Nottem, gnp, Airpassengers), transform them into stationarity.
...
abline(v=1920:1925,col=4,lty=3)
```
```{r}
monthplot(nottem)
```
...
start(gnp)        #Starting point ...
end(gnp)          #Ending point ...
frequency(gnp)    # No. of (quarters) in a (year)
length(gnp)       #No. of observations 
#(end(gnp)[1]-start(gnp)[1]+1)*4 -3
```

### Is Variance constant?

```{r}
w1 <- window(gnp,start=1947,end=1960)
w2 <- window(gnp,start=1960,end=1980)
w3 <- window(gnp,start=1980,end=2000)

par(mfrow=c(1,3))
plot(w1,ylim=c(min(gnp),max(gnp)))
plot(w2,ylim=c(min(gnp),max(gnp)))
plot(w3,ylim=c(min(gnp),max(gnp)))

par(mfrow=c(1,3))
boxplot(w1,ylim = c(min(gnp),max(gnp)))
boxplot(w2,ylim = c(min(gnp),max(gnp)))
boxplot(w3,ylim = c(min(gnp),max(gnp)))
```
...
```
```{r}
lnw1 <- window(lngnp,start=1947,end=1960)
lnw2 <- window(lngnp,start=1960,end=1980)
lnw3 <- window(lngnp,start=1980,end=2000)

par(mfrow=c(1,3))
plot(lnw1,ylim=c(min(lngnp),max(lngnp)))
plot(lnw2,ylim=c(min(lngnp),max(lngnp)))
plot(lnw3,ylim=c(min(lngnp),max(lngnp)))

par(mfrow=c(1,3))
boxplot(lnw1,ylim=c(min(lngnp),max(lngnp)))
boxplot(lnw2,ylim=c(min(lngnp),max(lngnp)))
boxplot(lnw3,ylim=c(min(lngnp),max(lngnp)))
```
...
#### Nop, gnp series has been Seasonally adjusted
...
```{r}
par(mfrow=c(1,2))
acf(lngnp,ylim=c(-1,1),lag.max=60,lwd=2)
pacf(lngnp,ylim=c(-1,1),lag.max=60,lwd=2)
par(mfrow=c(1,1))
```
...
d1lngnp=diff(lngnp) #show difln equiv tasaincremrelativo ln(xt/xt-1)=(xt-xt-1)/xt-1
plot(d1lngnp)
abline(h=0)
abline(h=mean(d1lngnp), col = 2)
```
...
par(mfrow=c(1,2))
acf(d1lngnp,ylim=c(-1,1),lag.max=60,lwd=2)
pacf(d1lngnp,ylim=c(-1,1),lag.max=60,lwd=2)
par(mfrow=c(1,1))
```

### Summary: Series d1lngnp has constant mean and variance.ACF decaying rapidly towards 0. => Stationarity achieved

$$W_t=(1-B)\log X_t=\log X_t-\log X_{t-1}$$
```{r}
par(mfrow=c(1,2))
plot(gnp,main="X_t")
plot(d1lngnp,main="W_t=(1-B)log(X_t)")
abline(h=0)
par(mfrow=c(1,1))
